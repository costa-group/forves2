...
(* Calling follow_in_smap with valid bindings and a valid smap_value 
   cannot return None *)
Lemma valid_follow_not_none: forall  idx sv sb ops,
valid_sstack_value  idx sv ->
valid_bindings  idx sb ops ->
follow_in_smap sv idx sb <> None.
Proof.
intros  idx sv sb. revert  idx sv.
induction sb as [| h t IH].
- intros  idx sv ops Hvalid_sstack Hvalid_bindings.
  destruct sv as [val|var|fvar] eqn: eq_sv.
...
Lemma follow_to_val_eval_eq: forall sv idx sb v d stk mem strg ext v',
follow_to_val sv idx sb = Some v ->
eval_sstack_val' d sv stk mem strg ext idx sb evm_stack_opm = Some v' ->
v = v'.
Proof.
intros sv idx sb v d stk mem strg ext v'.
...
Lemma follow_to_val_args_eval_eq: forall args idx sb vargs maxidx stk mem strg ext vargs',
follow_to_val_args args idx sb = Some vargs ->
map_option (fun sv' : sstack_val =>
             eval_sstack_val' maxidx sv' stk mem strg ext idx sb
               evm_stack_opm) args = Some vargs' ->
vargs = vargs'.
Proof.
induction args as [|h t IH].
- intros idx sb vargs maxidx stk mem strg ext vargs'.
...
- intros idx sb vargs maxidx stk mem strg ext vargs'.
  ...
  destruct (follow_to_val_args t idx sb) as [tv|] eqn: eq_follow_t;
    try discriminate.
  simpl in Hmapo.
  destruct (eval_sstack_val' maxidx h stk mem strg ext idx sb evm_stack_opm)
    as [hv'|] eqn: eq_eval_h; try discriminate.
  destruct (map_option (fun sv' : sstack_val =>
             eval_sstack_val' maxidx sv' stk mem strg ext idx sb
               evm_stack_opm) t) as [tv'|] eqn: eq_mapo_t;
    try discriminate.
  pose proof (IH idx sb tv maxidx stk mem strg ext tv' eq_follow_t
    eq_mapo_t) as eq_tv'.
  injection Hmapo as eq_vargs'. rewrite <- eq_vargs'.
  injection Hfollow as eq_vargs. rewrite <- eq_vargs.
  rewrite <- eq_tv'.
  pose proof (follow_to_val_eval_eq h idx sb hv maxidx stk mem strg ext hv'
...
(* Used to unfold eval_sstack_val' without applying any further 
   simplification *)
Lemma eval_sstack_val'_one_step: forall d' sv model mem strg exts maxidx sb ops,
eval_sstack_val' (S d') sv model mem strg exts maxidx sb ops =
      match follow_in_smap sv maxidx sb with
      | None => None
...
Lemma eval'_maxidx_indep: forall d sv stk mem strg ext n m sb ops v,
eval_sstack_val' d sv stk mem strg ext n sb ops = Some v ->
eval_sstack_val' d sv stk mem strg ext m sb ops = Some v.
Proof.
intros d sv stk mem strg ext n m sb ops v Heval.
destruct d as [|d']; try discriminate.
...
Lemma eval_sstack_val'_freshvar: forall n sv stk mem strg ext m sb ops idx,
eval_sstack_val' n sv stk mem strg ext m sb ops = 
eval_sstack_val' n (FreshVar idx) stk mem strg ext m 
  ((idx, SymBasicVal sv) :: sb) ops.
Proof.
intros n sv stk mem strg ext m sb ops idx.
destruct n as [|n'] eqn: eq_n.
- reflexivity. 
- rewrite -> eval_sstack_val'_one_step. 
  rewrite -> eval_sstack_val'_one_step.
...
Lemma eval_sstack_def: forall (sstk: sstack) (maxidx: nat) (sb: sbindings) 
  (model : assignment) (mem: memory) (strg: storage) (ext: externals) 
  (ops: stack_op_instr_map),
eval_sstack sstk maxidx sb model mem strg ext ops = 
map_option (fun sv => eval_sstack_val sv model mem strg ext maxidx sb ops) sstk.
Proof.
intuition.
Qed.
...
Lemma eval'_maxidx_indep_eq:
  forall (d : nat) (sv : sstack_val) (model : assignment) 
    (mem : memory) (strg : storage) (ext : externals) 
    (n m : nat) (sb : sbindings) (ops : stack_op_instr_map),
  eval_sstack_val' d sv model mem strg ext n sb ops = 
  eval_sstack_val' d sv model mem strg ext m sb ops.
Proof.
intros d sv stk mem strg ext n m sb ops.
...
(* Re-check *)
(*
Lemma eq_funs_len: forall maxidx (model: assignment) mem strg ext n sb1 sb2 ops ,
...
forall sv r, 
  (fun sv' : sstack_val =>
     eval_sstack_val' maxidx sv' model mem strg ext n sb1 ops) sv = Some r ->
  (fun sv' : sstack_val =>
     eval_sstack_val' maxidx sv' model mem strg ext n sb2 ops) sv = Some r.
Proof.
intros maxidx model mem strg ext n sb1 sb2 ops Hpreserv.
intros sv r Heval_f1.
simpl in Heval_f1.
simpl.
pose proof (Hpreserv sv r Heval_f1).
assumption.
Qed.
*) (* It's the same as map_option_fun_sstack *)


Lemma map_option_fun_sstack: forall maxidx model mem strg ext n sb1 sb2 ops,
(forall (sv : sstack_val) (v : EVMWord),
           (*instk_height = length stk ->*)
           eval_sstack_val' maxidx sv model mem strg ext n sb1 ops = Some v ->
           eval_sstack_val' maxidx sv model mem strg ext n sb2 ops = Some v) ->
...
Lemma map_option_sstack: forall maxidx model mem strg ext n sb1 sb2 ops 
  args vargs,
map_option (fun sv' : sstack_val =>
              eval_sstack_val' maxidx sv' model mem strg ext n sb1 ops)
              args = Some vargs ->
(forall (sv : sstack_val) (v : EVMWord),
           (* instk_height = length stk ->*)
           eval_sstack_val' maxidx sv model mem strg ext n sb1 ops = Some v ->
...
(*instk_height = length stk ->*)
map_option (fun sv' : sstack_val =>
              eval_sstack_val' maxidx sv' model mem strg ext n sb2 ops)
            args = Some vargs.
Proof.
intros maxidx model mem strg ext n sb1 sb2 ops args vargs Hmapo
  Hpreserv.
pose proof (map_option_fun_sstack maxidx model mem strg ext n sb1 sb2 ops 
  Hpreserv) as Hfunct_equiv.
pose proof (map_option_preserv_functs 
  ((fun sv' : sstack_val =>
                eval_sstack_val' maxidx sv' model mem strg ext n sb1 ops))
  (fun sv' : sstack_val =>
   eval_sstack_val' maxidx sv' model mem strg ext n sb2 ops) args vargs
  Hmapo Hfunct_equiv).
assumption.
Qed.
...
Lemma map_option_inst_mem_update: forall maxidx model mem strg ext n sb1 sb2 ops 
  smem mem_updates, 
...
map_option (instantiate_memory_update
             ...
             smem = Some mem_updates.
Proof. 
intros maxidx model mem strg ext n sb1 sb2 ops smem mem_updates
  Hmapo Hpreserv.
pose proof (map_option_fun_sstack maxidx model mem strg ext n sb1 sb2 ops
  Hpreserv) as Hfunct_equiv.
...
Lemma map_option_inst_strg_update: forall maxidx model mem strg ext n sb1 sb2 ops 
  sstrg strg_updates, 
...
map_option (instantiate_storage_update
             ...
             sstrg = Some strg_updates.
Proof. 
intros maxidx model mem strg ext n sb1 sb2 ops sstrg strg_updates
  Hmapo Hpreserv.
pose proof (map_option_fun_sstack maxidx model mem strg ext n sb1 sb2 ops 
  Hpreserv) as Hfunct_equiv.
...
Lemma eval'_then_valid_sstack_value: forall d sv model mem strg ext maxidx sb ops
  v n,
eval_sstack_val' (S d) sv model mem strg ext maxidx sb ops = Some v ->
valid_bindings n sb ops ->
(*instk_height = length stk ->*)
valid_sstack_value n sv.
Proof.
intros d sv model mem strg ext maxidx sb ops v n.
intros Heval Hvalid.
destruct sv as [val|var|fvar].
- simpl. intuition.
- simpl. auto. 
...
- simpl.
  ...
  destruct (follow_in_smap (FreshVar fvar) maxidx sb) as [fsm|] eqn: eq_follow;
    try discriminate.
  pose proof (follow_then_fvar_lt maxidx sb ops n fvar fsm
    Hvalid eq_follow).
  assumption.
Qed.
...
Lemma eval_sstack_val'_diff:
  forall (fvar idx : nat) (model : assignment) (mem : memory) 
    (strg : storage) (ext : externals) (d a b: nat) 
    (smapv : smap_value) (ops : stack_op_instr_map) 
    (sb : sbindings),
  (idx =? fvar) = false ->
  eval_sstack_val' (S d) (FreshVar fvar) model mem strg ext a 
    ((idx, smapv) :: sb) ops =
  eval_sstack_val' (S d) (FreshVar fvar) model mem strg ext b sb ops.
Proof.
intros fvar idx model mem strg ext d a b smapv ops sb.
intros Hfvar_idx.
...
Lemma eval_sstack_val_const: forall v model mem strg ext maxidx sb ops,
eval_sstack_val (Val v) model mem strg ext maxidx sb ops = Some v.
Proof.
intros.
...
Lemma eval_sstack_val'_const: forall n v model mem strg ext idx sb ops,
eval_sstack_val' (S n) (Val v) model mem strg ext idx sb ops = Some v.
Proof.
intros n v stk mem strg ext idx sb ops.
simpl. rewrite -> follow_in_smap_val.
reflexivity.
Qed.


Lemma eval_sstack_val'_invar: forall n var model mem strg ext idx sb ops,
eval_sstack_val' (S n) (InVar var) model mem strg ext idx sb ops =
  Some (model var).
Proof.
intros n var model mem strg ext idx sb ops.
simpl. rewrite -> follow_in_smap_instackvar.
reflexivity.
Qed.


Lemma eval_sstack_val'_extend_sb: forall n model mem strg ext idx sb sb' ops prefix,
valid_bindings idx sb ops ->
sb = prefix ++ sb' ->
forall sv v,
eval_sstack_val' n sv model mem strg ext idx sb' ops = Some v ->
eval_sstack_val' n sv model mem strg ext idx sb ops = Some v.
Proof.
intros n model mem strg ext idx sb sb' ops prefix.
...
- rewrite -> eval_sstack_val'_const.
  ...
  assumption.
- rewrite -> eval_sstack_val'_invar.
  rewrite -> eval_sstack_val'_invar in Heval.
  assumption.
- rewrite -> eval_sstack_val'_one_step in Heval. 
  destruct (follow_in_smap (FreshVar fvar) idx sb') as [follow_smap|] 
    eqn: Hfollow; try discriminate.
  rewrite -> eval_sstack_val'_one_step.
  pose proof (follow_in_smap_prefix fvar idx idx sb' follow_smap
...
Lemma eval_sstack_val'_extend_sb_indep: forall n d model mem strg ext 
  m1 m2 sb sb' ops prefix,
valid_bindings n sb ops ->
sb = prefix ++ sb' ->
forall sv v, 
eval_sstack_val' d sv model mem strg ext m1 sb' ops = Some v ->
eval_sstack_val' d sv model mem strg ext m2 sb ops = Some v.
Proof.
intros n d model mem strg ext m1 m2 sb sb' ops prefix.
intros Hvalid Hprefix sv v Heval.
rewrite -> eval'_maxidx_indep_eq with (m:=n) in Heval.
pose proof (eval_sstack_val'_extend_sb d model mem strg ext n
...
Lemma lambda_eval'_maxid_indep_eq: forall d model mem strg ext n m sb1 ops,
(fun sv : sstack_val =>
             eval_sstack_val' d sv model mem strg ext n sb1 ops) = 
(fun sv : sstack_val =>
             eval_sstack_val' d sv model mem strg ext m sb1 ops).
Proof.
intros.
...
Lemma lambda_eval'_eq: forall d model mem strg ext m1 m2 sb1 sb2 ops,
(forall (sv : sstack_val) (v : EVMWord),
    eval_sstack_val' d sv model mem strg ext m1 sb1 ops = Some v ->
    eval_sstack_val' d sv model mem strg ext m2 sb2 ops = Some v) ->
(forall sv v,
   (fun sv : sstack_val =>
      eval_sstack_val' d sv model mem strg ext m1 sb1 ops) sv = Some v ->
   (fun sv : sstack_val => 
      eval_sstack_val' d sv model mem strg ext m2 sb2 ops) sv = Some v).
Proof.
intuition.
Qed.
...
Lemma map_option_preserv_prefix: forall d model mem strg ext sb1 sb ops
  args vargs prefix n m1 m2,
map_option (fun sv : sstack_val =>
  eval_sstack_val' d sv model mem strg ext m1 sb1 ops) args = Some vargs ->
sb = prefix ++ sb1 ->
valid_bindings n sb ops ->
map_option (fun sv : sstack_val => 
  eval_sstack_val' d sv model mem strg ext m2 sb ops) args = Some vargs.
Proof.
intros d model mem strg ext sb1 sb ops args vargs prefix n m1 m2.
intros Hmapo Hprefix Hvalid.
pose proof (eval_sstack_val'_extend_sb_indep  n d model mem strg 
  ext m1 m2 sb sb1 ops prefix Hvalid Hprefix) as Heq_evals.
pose proof (lambda_eval'_eq d model mem strg ext m1 m2 sb1 sb ops Heq_evals) as
  Hlambda_eq.
pose proof (map_option_preserv_functs
  (fun sv0 : sstack_val =>
     eval_sstack_val' d sv0 model mem strg ext m1 sb1 ops)
  (fun sv0 : sstack_val =>
     eval_sstack_val' d sv0 model mem strg ext m2 sb ops) 
...
Lemma map_option_preserv_prefix_inst_mem: forall d model mem strg ext sb1 sb ops
  smem mem_updates prefix  n m1 m2,
map_option (instantiate_memory_update (fun sv : sstack_val =>
  eval_sstack_val' d sv model mem strg ext m1 sb1 ops)) smem = 
    Some mem_updates ->
sb = prefix ++ sb1 ->
valid_bindings  n sb ops ->
map_option (instantiate_memory_update (fun sv : sstack_val =>
  eval_sstack_val' d sv model mem strg ext m2 sb ops)) smem = 
    Some mem_updates.
Proof.
intros d model mem strg ext sb1 sb ops smem mem_updates prefix  n 
  m1 m2 Hmapo Hprefix Hvalid.
pose proof (eval_sstack_val'_extend_sb_indep  n d model mem strg 
  ext m1 m2 sb sb1 ops prefix Hvalid Hprefix) as Heq_evals.
pose proof (lambda_eval'_eq d model mem strg ext m1 m2 sb1 sb ops Heq_evals)
  as Heq_lambdas.
pose proof (same_instantiate_memory_update
  (fun sv0 : sstack_val =>
               eval_sstack_val' d sv0 model mem strg ext m1 sb1 ops)
...
Lemma map_option_preserv_prefix_inst_strg: forall d model mem strg ext sb1 sb ops
  sstrg strg_updates prefix  n m1 m2,
map_option (instantiate_storage_update (fun sv : sstack_val =>
  eval_sstack_val' d sv model mem strg ext m1 sb1 ops)) sstrg 
    = Some strg_updates ->
sb = prefix ++ sb1 ->
valid_bindings  n sb ops ->
map_option (instantiate_storage_update (fun sv : sstack_val =>
  eval_sstack_val' d sv model mem strg ext m2 sb ops)) sstrg 
    = Some strg_updates.
Proof.
intros d model mem strg ext sb1 sb ops sstrg strg_updates prefix  n 
  m1 m2 Hmapo Hprefix Hvalid.
pose proof (eval_sstack_val'_extend_sb_indep  n d model mem strg 
  ext m1 m2 sb sb1 ops prefix Hvalid Hprefix) as Heq_evals.
pose proof (lambda_eval'_eq d model mem strg ext m1 m2 sb1 sb ops Heq_evals)
...
pose proof (map_option_preserv_functs
  (instantiate_storage_update (fun sv : sstack_val =>
     eval_sstack_val' d sv model mem strg ext m1 sb1 ops))
  (instantiate_storage_update (fun sv : sstack_val =>
     eval_sstack_val' d sv model mem strg ext m2 sb ops)) 
...
Lemma eval_sstack_val'_follow_in_smap: forall d sv model mem strg ext m
  sb ops v maxidx' sb' n sv_plain ,
valid_bindings  n sb ops ->  
eval_sstack_val' d sv model mem strg ext n sb ops 
  = Some v ->
follow_in_smap sv n sb = Some (FollowSmapVal sv_plain maxidx' sb') ->
eval_sstack_val' d (FreshVar n) model mem strg ext m 
  ((n, sv_plain) :: sb) ops = Some v.
Proof.
intros d sv model mem strg ext m sb ops v maxidx' sb' n sv_plain 
   Hvalid Heval Hfollow.
...
- simpl in Heval. 
  ...
  + (* SymOp op args *)
    ...
    destruct (map_option
               (fun sv' : sstack_val =>
               eval_sstack_val' d' sv' model mem strg ext maxid1 sb1 ops) args)
      as [vargs|] eqn: eq_mapo; try discriminate.
    rewrite <- Heval.
    rewrite -> lambda_eval'_maxid_indep_eq with (m:=maxid1).
    pose proof (map_option_preserv_prefix d' model mem strg ext sb1 sb ops
      args vargs prefix  n 
  ...
  + (* SymMLOAD offset smem *)
    destruct (map_option
            (instantiate_memory_update
               (fun sv : sstack_val =>
                eval_sstack_val' d' sv model mem strg ext maxid1 sb1 ops))
            smem) as [mem_updates|] eqn: eq_mapo; try discriminate.
    destruct (eval_sstack_val' d' offset model mem strg ext maxid1 sb1 ops)  
      as [offsetv|] eqn: eq_eval_offset; try discriminate.
    rewrite <- Heval.
    pose proof (map_option_preserv_prefix_inst_mem d' model mem strg ext sb1
      sb ops smem mem_updates prefix
       n maxid1 n eq_mapo eq_prefix Hvalid) as eq_mapo2.
    rewrite -> eq_mapo2.
    pose proof (eval_sstack_val'_extend_sb_indep  n d' model mem 
      strg ext maxid1 n sb sb1 ops prefix Hvalid eq_prefix
  ...
  + (* SymSLOAD key sstrg *)
    ...
    destruct (eval_sstack_val' d' key model mem strg ext maxid1 sb1 ops)
      as [keyv|] eqn: eq_eval_key; try discriminate.
    rewrite <- Heval.
    pose proof (map_option_preserv_prefix_inst_strg d' model mem strg ext sb1
  ...
  + (* SymSHA3 offset size smem *)
    ...
    destruct (eval_sstack_val' d' offset model mem strg ext maxid1 sb1 ops)
      as [offsetv|] eqn: eq_eval_offset; try discriminate.
    destruct (eval_sstack_val' d' size model mem strg ext maxid1 sb1 ops)
    ...
    rewrite -> eq_mapo2.
    pose proof (eval_sstack_val'_extend_sb_indep  n d' model mem 
      strg ext maxid1 n sb sb1 ops prefix Hvalid eq_prefix offset offsetv 
...
Lemma eval_eq_prefix: forall  n n' sb sb' ops sv model mem strg 
  ext d d' prefix v1 v2,
valid_bindings  n sb ops ->
n > n' ->
sb = prefix ++ sb' ->  
eval_sstack_val' d  sv model mem strg ext n  sb  ops = Some v1 ->
eval_sstack_val' d' sv model mem strg ext n' sb' ops = Some v2 ->
v1 = v2.
Proof.
intros  n n' sb sb' ops sv model mem strg ext d d' prefix v1 v2.
intros Hvalid n_gt_n' Hprefix Heval1 Heval2.
rewrite -> eval'_maxidx_indep_eq with (m:=n) in Heval2.  
apply eval_sstack_val'_extend_sb with (sb:=sb)
...
Lemma eval'_succ_then_nonzero: forall maxidx sv model mem strg ext idx sb ops v,
eval_sstack_val' maxidx sv model mem strg ext idx sb ops = Some v ->
exists n, maxidx = S n.
Proof.
intros maxidx sv model mem strg ext idx sb ops v.
intros Heval.
