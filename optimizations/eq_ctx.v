...
(* EQ(X,Y) = 1   if ctx |- X=Y *)
Definition optimize_eq_ctx_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
match val with
| SymOp EQ [arg1; arg2] => 
  match chk_eq_wrt_ctx ctx arg1 arg2 with
    | false => match chk_neq_wrt_ctx ctx arg1 arg2 with
...
Lemma optimize_eq_ctx_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_eq_ctx_sbinding.
Proof.
  unfold opt_smapv_valid_snd.
  intros ctx n fcmp sb val val' flag.
  intros Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
  unfold optimize_eq_ctx_sbinding in Hoptm_sbinding.
  ...
  destruct (chk_eq_wrt_ctx ctx arg1 arg2); 
    try (injection Hoptm_sbinding as eq_val' eq_flag;
    rewrite <- eq_val';
    simpl; intuition).
  destruct (chk_neq_wrt_ctx ctx arg1 arg2); 
    try (injection Hoptm_sbinding as eq_val' eq_flag;
    rewrite <- eq_val';
    simpl; intuition).
Qed.
...
Lemma optimize_eq_ctx_sbinding_snd:
opt_sbinding_snd optimize_eq_ctx_sbinding.
Proof.
unfold opt_sbinding_snd.
intros val val' tools sb maxidx ctx idx flag Hvalid Hoptm_sbinding.
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_eq_ctx_sbinding)
  (tools:=tools)(flag:=flag)(ctx:=ctx); try assumption.
  apply optimize_eq_ctx_sbinding_smapv_valid.
...
- (* evaluation is preserved *) 
  ...
  destruct r2 as [|arg3 r3] eqn: eq_r2; 
    try inject_rw Hoptm_sbinding eq_val'.
  destruct (chk_eq_wrt_ctx ctx arg1 arg2) eqn: eq_chk_eq.
    + unfold eval_sstack_val in Heval_orig.
      simpl in Heval_orig.
      rewrite -> PeanoNat.Nat.eqb_refl in Heval_orig.
      rewrite -> evm_stack_opm_EQ in Heval_orig.
      ...
      apply chk_eq_wrt_ctx_snd with (model:=model)(mem:=mem)(strg:=strg)
        (exts:=exts)(maxidx:=maxidx)(sb:=sb)(ops:=evm_stack_opm) in eq_chk_eq;
        try assumption.
      destruct eq_chk_eq as [vv [Heval_arg1 Heval_arg2]].
      ...
      simpl in Heval_orig.
      rewrite -> weqb_eq in Heval_orig; try reflexivity.
      rewrite <- Heval_orig.
      ...
      reflexivity.
    + destruct (chk_neq_wrt_ctx ctx arg1 arg2) eqn: eq_chk_neq;
        try inject_rw Hoptm_sbinding eq_val'.
      unfold eval_sstack_val in Heval_orig.
      simpl in Heval_orig.
      rewrite -> PeanoNat.Nat.eqb_refl in Heval_orig.
      ...
      apply chk_neq_wrt_ctx_snd with (model:=model)(mem:=mem)(strg:=strg)
        (exts:=exts)(maxidx:=maxidx)(sb:=sb)(ops:=evm_stack_opm) in eq_chk_neq;
        try assumption.
      destruct eq_chk_neq as [v1 [v2 [Heval_arg1 [Heval_arg2 Hneq]]]].
      unfold eval_sstack_val in Heval_arg1.
      unfold eval_sstack_val in Heval_arg2.
      apply eval_sstack_val'_preserved_when_depth_extended in eq_eval_arg1.
      rewrite -> eval'_maxidx_indep_eq with (m:=maxidx) in eq_eval_arg1.
