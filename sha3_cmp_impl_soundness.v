...
Qed.
  ...
  Lemma update_out_of_slot_snd_1:
    forall (ctx: ctx_t) (min max: N) (u: memory_update sstack_val) (smem: smemory) (maxidx : nat) (sb : sbindings) (ops : stack_op_instr_map) (model : assignment) (mem : memory) (strg : storage) (exts : externals) (mem1 mem1' : memory) (offset size : EVMWord),
      ...
      eval_smemory (u::smem) maxidx sb model mem strg exts ops = Some mem1' ->
      ((wordToN offset) >= min)%N ->
      (((wordToN offset)+(wordToN size)) <= max)%N ->
      update_out_of_slot ctx u min max maxidx sb ops = true ->
      exists v,
  ...
  Proof.
    ...
    destruct (map_option (instantiate_memory_update (fun sv : sstack_val => eval_sstack_val sv model mem strg exts maxidx sb ops)) (u::smem)) as [u_updates|] eqn:E_mo_u_updates; try discriminate.
    injection H_eval_u_smem as H_mem1'.

    pose proof (map_option_split (memory_update sstack_val) (memory_update EVMWord) (instantiate_memory_update (fun sv : sstack_val => eval_sstack_val sv model mem strg exts maxidx sb ops)) smem u_updates u E_mo_u_updates) as E_mo_u_updates_0.
    ...
    + unfold update_out_of_slot in H_update_oos.
      ...
      pose proof (eval_sstack_val_succ sb uvalue model mem strg exts maxidx ops H_valid_uvalue H_valid_bs) as H_eval_uvalue.
...
Qed.
  ...
  Lemma update_out_of_slot_snd_2:
    forall (ctx: ctx_t) (smem1 smem2: smemory) (u: memory_update sstack_val) (maxidx : nat) (sb : sbindings) (ops : stack_op_instr_map) (model : assignment) (mem : memory) (strg : storage) (exts : externals) (mem1 mem2 mem1' mem2' : memory) (offset size : EVMWord),
      ...
      valid_smemory_update maxidx u ->
      eval_smemory smem1 maxidx sb model mem strg exts ops = Some mem1 ->
      eval_smemory smem2 maxidx sb model mem strg exts ops = Some mem2 ->
      eval_smemory (u::smem1) maxidx sb model mem strg exts ops = Some mem1' ->
      eval_smemory (u::smem2) maxidx sb model mem strg exts ops = Some mem2' ->
  ...
  Proof.
    ...
    destruct (map_option (instantiate_memory_update (fun sv : sstack_val => eval_sstack_val sv model mem strg exts maidx sb ops)) smem2) as [updates2|] eqn:E_mo_updates2; try discriminate.
    injection H_eval_smem2 as H_mem2.
    
    unfold eval_smemory in H_eval_u_smem1.
    destruct (map_option (instantiate_memory_update (fun sv : sstack_val => eval_sstack_val sv model mem strg exts maidx sb ops)) (u::smem1)) as [u_updates1|] eqn:E_mo_u_updates1; try discriminate.
    injection H_eval_u_smem1 as H_mem1'.

    unfold eval_smemory in H_eval_u_smem2.
    destruct (map_option (instantiate_memory_update (fun sv : sstack_val => eval_sstack_val sv model mem strg exts maidx sb ops)) (u::smem2)) as [u_updates2|] eqn:E_mo_u_updates2; try discriminate.
    injection H_eval_u_smem2 as H_mem2'.

    pose proof (map_option_split (memory_update sstack_val) (memory_update EVMWord) (instantiate_memory_update (fun sv : sstack_val => eval_sstack_val sv model mem strg exts maidx sb ops)) smem1 u_updates1 u E_mo_u_updates1) as E_mo_u_updates1_0.
    ...
    pose proof (map_option_split (memory_update sstack_val) (memory_update EVMWord) (instantiate_memory_update (fun sv : sstack_val => eval_sstack_val sv model mem strg exts maidx sb ops)) smem2 u_updates2 u E_mo_u_updates2) as E_mo_u_updates2_0.
  ...
  Lemma remove_out_of_slot'_snd_1:
    forall  (min max: N) (smem smem_r: smemory) (ctx: ctx_t) (maxidx : nat) (sb : sbindings) (ops : stack_op_instr_map)
      ...
      valid_smemory maxidx smem ->
      eval_smemory smem maxidx sb model mem strg exts ops = Some mem1 ->
      eval_smemory smem_r maxidx sb model mem strg exts ops = Some mem1' ->
      ((wordToN offset) >= min)%N ->
      (((wordToN offset)+(wordToN size)) <= max)%N ->
      remove_out_of_slot' ctx smem min max maxidx sb ops = smem_r ->
      exists v,
        (mload' mem1 offset (wordToNat size)) = v /\
          (mload' mem1' offset (wordToNat size)) = v.
  Proof.
...
Lemma remove_out_of_slot'_snd:
  forall (ctx: ctx_t) (min max: N) (smem smem_r: smemory) (maxidx : nat) (sb : sbindings) (ops : stack_op_instr_map) (model : assignment) (mem : memory) (strg : storage) (exts : externals) (mem1 mem1' : memory)
    ...
    remove_out_of_slot' ctx smem min max maxidx sb ops = smem_r ->
    exists v,
      (get_sha3_info_op (get_keccak256_exts exts)) (wordToNat size)
        (mload' mem1 offset (wordToNat size)) = v /\
        (get_sha3_info_op (get_keccak256_exts exts)) (wordToNat size)
          (mload' mem1' offset (wordToNat size)) = v.
Proof.
  ...
  rewrite H_snd_1_0.
  rewrite H_snd_1_1.
  exists (get_sha3_info_op (get_keccak256_exts exts) (wordToNat size) v').
  split; reflexivity.
Qed.
...
Qed.
    ...
    forall (ctx: ctx_t) (soffset ssize : sstack_val) (smem smem': smemory) (maxidx : nat) (sb : sbindings) (ops : stack_op_instr_map),
      ...
      forall (model : assignment) (mem : memory) (strg : storage) (exts : externals),
        ...
        exists
          ...
          eval_smemory smem maxidx sb model mem strg exts ops = Some mem1 /\
            eval_smemory smem' maxidx sb model mem strg exts ops = Some mem1' /\
            eval_sstack_val soffset model mem strg exts maxidx sb ops = Some offset /\
            eval_sstack_val ssize model mem strg exts maxidx sb ops = Some size /\
            (get_sha3_info_op (get_keccak256_exts exts)) (wordToNat size)
              (mload' mem1 offset (wordToNat size)) = v /\
            (get_sha3_info_op (get_keccak256_exts exts)) (wordToNat size)
              (mload' mem1' offset (wordToNat size)) = v.
  Proof.
    ...
    pose proof (eval_sstack_val_succ sb soffset model mem strg exts maxidx ops H_valid_soffset H_valid_sb) as H_eval_soffset.
    destruct H_eval_soffset as [soffset_v H_eval_soffset].

    pose proof (eval_sstack_val_succ sb ssize model mem strg exts maxidx ops H_valid_ssize H_valid_sb) as H_eval_ssize.
    ...
    eval_smemory smem maxidx sb model mem strg exts ops = Some mem0 /\
    eval_smemory smem' maxidx sb model mem strg exts ops = Some mem1'0 /\
    eval_sstack_val soffset model mem strg exts maxidx sb ops = Some offset /\
    eval_sstack_val ssize model mem strg exts maxidx sb ops = Some size /\
    (get_sha3_info_op (get_keccak256_exts exts)) (wordToNat size) (mload' mem0 offset (wordToNat size)) = v /\ (get_sha3_info_op (get_keccak256_exts exts)) (wordToNat size) (mload' mem1'0 offset (wordToNat size)) = v
           ).
    (* proof of assert *)
    ...
    exists ((get_sha3_info_op (get_keccak256_exts exts)) (wordToNat ssize_v) (mload' mem1 soffset_v (wordToNat ssize_v))).
  ...
  Proof.
    ...
    destruct (sstack_val_cmp d' ctx soffset1 soffset2 maxidx1 sb1 maxidx2 sb2 ops && sstack_val_cmp d' ctx ssize1 ssize2 maxidx1 sb1 maxidx2 sb2 ops) eqn:H_eq_size_and_offset; try discriminate.
